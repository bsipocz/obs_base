...
class RawIngestTask(Task):
    ...
    **kwargs
    ...
    def run(self, files, *, pool: Optional[Pool] = None, processes: int = 1, run: Optional[str] = None,
            ingested: Optional[List[FileDataset]] = None):
        ...
        ingested : `list` [ `FileDataset` ], optional
            The list of datasets ingested, updated whenever dataset ingestion
            is committed.  Passing an empty list allows callers that catch
            exceptions to see what has been completed.
        ...
        for exposure in self.progress.wrap(exposureData, desc="Ingesting raw exposures"):
            ...
            try:
                ...
                if ingested is not None:
                    ingested.extend(datasets_for_exposure)
